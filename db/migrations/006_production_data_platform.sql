...
alter table spin_users
  drop constraint if exists spin_users_spin_accounting_check;

alter table spin_users
  add constraint spin_users_spin_accounting_check
  check (spins_earned = spins_available + spins_used);

create index if not exists spin_users_last_seen_idx
  on spin_users(last_seen_at desc);
...
alter table spin_daily_rollups
  drop constraint if exists spin_daily_rollups_pkey;

alter table spin_daily_rollups
  add constraint spin_daily_rollups_pkey
  primary key (campaign_id, metric_day, metric_shard);

create index if not exists spin_daily_rollups_day_idx
  on spin_daily_rollups(metric_day desc);
...
select
  (hashtextextended(id::text, 0) & 63::bigint)::smallint,
  count(*)::bigint
from spin_users
...
select
  encode(digest(lower(wallet_address), 'sha256'), 'hex'),
  id,
  user_id,
  wallet_submitted_at
from spin_wins
...
create index if not exists spin_events_created_idx
  on spin_events(created_at);
...
values ('006_production_data_platform')
