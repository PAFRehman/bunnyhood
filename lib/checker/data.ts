...
export function requireCheckerWallet(value: string) {
  ...
  if (!EVM_WALLET.test(wallet)) {
    throw new HttpError(400, "Enter a valid EVM wallet address.", "BAD_CHECKER_WALLET");
  }
  return wallet;
}

function parseWalletList(value: string, eligibilityType: CheckerEligibility) {
  ...
  for (const rawValue of withoutComments.split(/[\s,;]+/)) {
    ...
    if (!token || /^(?:wallet|wallet_address|address)$/i.test(token)) continue;
  ...
  return [...wallets].map((walletAddress) => ({ walletAddress, eligibilityType }));
}

export function parseCheckerImport(gtdValue: string, fcfsValue: string) {
  const gtd = parseWalletList(gtdValue, "GTD");
  const fcfs = parseWalletList(fcfsValue, "FCFS");
  const gtdWallets = new Set(gtd.map((entry) => entry.walletAddress));
  const conflict = fcfs.find((entry) => gtdWallets.has(entry.walletAddress));
  if (conflict) {
    throw new HttpError(
      400,
      `${conflict.walletAddress} appears in both GTD and FCFS.`,
      "CHECKER_IMPORT_CONFLICT",
    );
  }
  const entries = [...gtd, ...fcfs];
  if (!entries.length) {
    throw new HttpError(400, "Paste at least one GTD or FCFS wallet.", "EMPTY_CHECKER_IMPORT");
  }
...
async function checkerStatsWithSql(sql: SpinDb) {
  const rows = await sql<{ total: number; gtd: number; fcfs: number }[]>`
    select count(*)::integer as total,
      count(*) filter (where eligibility_type = 'GTD')::integer as gtd,
      count(*) filter (where eligibility_type = 'FCFS')::integer as fcfs
    from checker_wallets
  `;
  return rows[0] ?? { total: 0, gtd: 0, fcfs: 0 };
}
...
export async function findCheckerEligibility(walletValue: string) {
  const wallet = requireCheckerWallet(walletValue);
  await ensureCheckerSchema();
  const rows = await getDb()<Pick<CheckerWalletRow, "eligibility_type">[]>`
...
export async function listCheckerWallets(search = "") {
  await ensureCheckerSchema();
  const trimmed = search.trim().slice(0, 80).toLowerCase();
  const pattern = `%${trimmed}%`;
  const rows = await getDb()<CheckerWalletRow[]>`
    select wallet_address, eligibility_type, imported_at, updated_at
    from checker_wallets
    where ${trimmed} = '' or wallet_address like ${pattern}
...
export async function upsertCheckerWallets(entries: CheckerImport[]) {
  await ensureCheckerSchema();
  return inTransaction(async (sql) => {
    await sql`select pg_advisory_xact_lock(hashtext('bunny-hood-checker-import'))`;
    for (let offset = 0; offset < entries.length; offset += 500) {
      ...
      await sql.unsafe(
        `insert into checker_wallets (
          wallet_address, eligibility_type, imported_at, updated_at
        ) values ${values.join(",")}
...
export async function deleteCheckerWallet(walletValue: string) {
  const wallet = requireCheckerWallet(walletValue);
  await ensureCheckerSchema();
  const rows = await getDb()<Pick<CheckerWalletRow, "wallet_address">[]>`
    delete from checker_wallets
    where wallet_address = ${wallet}
    returning wallet_address
  `;
  if (!rows.length) {
    throw new HttpError(404, "That wallet is not in the checker.", "CHECKER_WALLET_NOT_FOUND");
  }
  return wallet;
}
