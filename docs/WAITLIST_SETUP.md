...
The public waitlist lives at `/waitlist`. It does not connect X and does not ask for a wallet signature or transaction. A browser completes two launch tasks, submits one verified public X post, then submits one unique EVM wallet. It receives an immutable join number and referral link. Live rank is ordered by points, then earliest join time:

```text
points = two required tasks + required verified post + successful referrals
```

Each new wallet that joins through a referral link adds one point to the referrer. Before wallet entry, X's public oEmbed service must verify the submitted `x.com/{user}/status/{id}` post. Each post ID and normalized X username can be used for only one waitlist entry. Existing entries created before this gate remain valid.
...
const HEADERS = [
  ...
  "X Username", "Bonus Submitted At", "Synced At", "Revision"
];
  ...
  try {
    ...
    if (lastRow > 1) {
      ...
      values.forEach((row, index) => {
        if (row[0]) existing[String(row[0])] = { row: index + 2, revision: Number(row[16] || 0) };
      });
    }
    ...
    body.events.forEach((eventItem) => {
      ...
      sheet.getRange(rowNumber, 1, 1, 17).setValues(row);
...
4. Create the prefilled BunnyHood post containing `@BunnysHood`, the reserved referral link, and its visible referral code, then submit its public X status URL without removing those fields.
5. The server verifies the post through X oEmbed and reserves its post ID and normalized X username. Neither can be used for another entry.
6. Enter a valid EVM wallet. The server accepts one entry per wallet and one entry per browser waitlist session.
7. Receive the permanent join number, live rank, three starting points, and referral link.
8. Search public rank by complete wallet address or referral code. Public results and the leaderboard mask wallet addresses; the admin ledger and Google Sheet retain the complete wallet.

## Important trust boundary

The follow/notification and engagement actions remain self-confirmed because X does not expose notification settings through the standard API. The required post must resolve through X's public oEmbed service and retain both `@BunnysHood` and the session's reserved referral code; its canonical author handle and post ID are globally reserved. Because the waitlist deliberately has no X login, this proves that a public post exists under that handle but does not provide a permanent numeric X identity binding. Entering a wallet also does not prove wallet ownership because no signature is requested.
...
Migration `012_waitlist_required_x_post` adds a reserved referral code to each completed session and creates `waitlist_join_posts`, which stores the verified public X handle and post required before a new wallet can join.
