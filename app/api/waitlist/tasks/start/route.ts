...
import { startWaitlistTask, type WaitlistTaskType } from "@/lib/waitlist/data";
...
export async function POST(request: Request) {
  try {
    ...
    await enforceRateLimit(sha256(`waitlist-task-start:${session.id}`), 20, 60);
    ...
    return json(await startWaitlistTask(session.id, body.task));
  } catch (error) {
