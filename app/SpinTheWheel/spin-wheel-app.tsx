              ...
              <section className="daily-campaign">
                ...
                <div className="spin-task-list">
                  ...
                  <article className={`post-point-task ${state.postTask?.completed ? "claimed" : ""}`}>
                    ...
                    {state.postTask?.completed ? <div className="spin-task-actions"><a href={state.postTask.postUrl} target="_blank" rel="noreferrer">Verified · +3 points</a></div> : <div className="post-task-controls"><a className="create-post-button" href={xShareUrl(state.campaign.shopPostText, referralLink)} target="_blank" rel="noreferrer">Create post on X</a><form onSubmit={submitPostTask}><label htmlFor="spin-post-url">Paste your published post URL</label><div><input id="spin-post-url" type="url" value={postUrl} onChange={(event) => setPostUrl(event.target.value)} placeholder="https://x.com/you/status/..." required /><button disabled={postTaskBusy}>{postTaskBusy ? "Verifying…" : "Verify +3"}</button></div></form></div>}
                  </article>
                </div>
              </section>
            )}
