...
if (!/CONGRATULATIONS!/.test(checkerPage) || !/GTD/.test(checkerPage) || !/FCFS/.test(checkerPage) || !/PUBLIC/.test(checkerPage) || !/round\.eligible \? "ELIGIBLE" : "NOT ELIGIBLE"/.test(checkerPage)) failures.push("The Checker round-by-round congratulations result is incomplete.");
if (!/checker-opening/.test(checkerPage) || !/bunny-hood-logo\.png/.test(checkerPage)) failures.push("The BunnyHood opening animation or corrected logo is missing.");
...
if (/WALLET INDEX · LIVE|01 \/ GUARANTEED|RH-CHAIN \/ 143|CHECKER UPDATES DAILY|waiting for the next list update/.test(checkerPage)) failures.push("Removed Checker labels remain public.");
