...
if (!/WRITE_BATCH_SIZE = 250/.test(checkerData) || !/insert into checker_wallets \$\{sql\(/.test(checkerData) || !/CHECKER_IMPORT_\$\{progress\.stage\.toUpperCase/.test(checkerData) || !/after\(async \(\) =>/.test(checkerAdminRoute)) failures.push("Checker bulk writes are not resilient or can still fail after a successful audit-independent commit.");
