...
if (!/create table if not exists last_dance_settings/.test(migration) || !/public_enabled boolean not null default false/.test(migration) || !/max_entries integer not null default 100/.test(migration) || !/add column if not exists mint_opens_at timestamptz/.test(migration)) failures.push("The Last Dance is not private by default or lacks its admin-controlled cap and mint schedule.");
...
if (!/CHAIN_ID = 4663/.test(lastDanceChain) || !/robinhoodchain\.blockscout\.com/.test(lastDanceChain) || !/ROBINHOOD_MAINNET_RPC_URL/.test(lastDanceChain) || !/ERC-721,ERC-404,ERC-1155/.test(lastDanceChain) || !/filter: "to \| from"/.test(lastDanceChain) || !/AbortSignal\.timeout/.test(lastDanceChain)) failures.push("The Last Dance chain proof is not fixed to bounded Robinhood mainnet any-NFT and any-transaction endpoints.");
...
if (!/DOWNLOAD GTD CARD/.test(lastDanceApp) || !/SHARE PASS ON X/.test(lastDanceApp) || !/YOUR WALLET WILL BE ADDED SOON/.test(lastDanceApp) || !/LastDanceMintCountdown/.test(lastDanceApp) || !/GO TO OPENSEA MINT/.test(lastDanceApp)) failures.push("The confirmed Last Dance entry is missing its download, X share, wallet notice, mint countdown, or OpenSea action.");
