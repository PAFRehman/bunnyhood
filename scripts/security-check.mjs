...
const lastDancePassPage = readFileSync(join(root, "app/TheLastDance/pass/[entryId]/page.tsx"), "utf8");
const lastDancePassRoute = readFileSync(join(root, "app/api/last-dance/pass/[entryId]/route.tsx"), "utf8");
const lastDanceAdminPage = readFileSync(join(root, "app/admin/last-dance/page.tsx"), "utf8");
const lastDanceAdminApp = readFileSync(join(root, "app/admin/last-dance/last-dance-admin-app.tsx"), "utf8");
const lastDanceAdminRoute = readFileSync(join(root, "app/api/admin/last-dance/route.ts"), "utf8");
const lastDanceExportRoute = readFileSync(join(root, "app/api/admin/last-dance/export/route.ts"), "utf8");
const lastDanceExport = readFileSync(join(root, "lib/last-dance/export.ts"), "utf8");
...
if (!/WRITE_BATCH_SIZE = 250/.test(checkerData) || !/values \$\{values\.join/.test(checkerData) || !/CHECKER_IMPORT_\$\{progress\.stage\.toUpperCase/.test(checkerData) || !/after\(async \(\) =>/.test(checkerAdminRoute)) failures.push("Checker bulk writes are not resilient or can still fail after a successful audit-independent commit.");
...
if (!/create table if not exists last_dance_settings/.test(migration) || !/public_enabled boolean not null default false/.test(migration) || !/max_entries integer not null default 100/.test(migration) || !/add column if not exists mint_opens_at timestamptz/.test(migration) || !/2026-09-09 15:30:00\+00/.test(migration)) failures.push("The Last Dance is not private by default or lacks its admin-controlled cap and verified GTD mint schedule.");
...
if (!/CHAIN_ID = 4663/.test(lastDanceChain) || !/robinhoodchain\.blockscout\.com/.test(lastDanceChain) || !/ROBINHOOD_MAINNET_RPC_URL_2/.test(lastDanceChain) || !/ROBINHOOD_MAINNET_RPC_URL_3/.test(lastDanceChain) || !/ROBINHOOD_BLOCKSCOUT_API_KEY_2/.test(lastDanceChain) || !/ROBINHOOD_BLOCKSCOUT_API_KEY_3/.test(lastDanceChain) || !/ERC-721,ERC-404,ERC-1155/.test(lastDanceChain) || !/filter: "to \| from"/.test(lastDanceChain) || !/AbortSignal\.timeout/.test(lastDanceChain)) failures.push("The Last Dance chain proof is not fixed to bounded Robinhood mainnet endpoints with three-provider failover.");
...
if (!/capacity/.test(lastDanceData) || !/setInterval/.test(lastDanceApp) || !/5_000/.test(lastDanceApp) || !/LIVE CAPACITY/.test(lastDanceApp)) failures.push("The Last Dance public page is missing its live claimed and remaining capacity counter.");
if (!/requireSpinAdmin/.test(lastDanceExportRoute) || !/recordAdminAction/.test(lastDanceExportRoute) || !/content-disposition/.test(lastDanceExportRoute) || !/text\/csv/.test(lastDanceExportRoute)) failures.push("The Last Dance full-data export is not protected, audited, or delivered as CSV.");
if (!lastDanceExport.includes("const safe = /^[=+\\-@]/") || !lastDanceExport.includes('write(output, "\\uFEFF")') || !/left join last_dance_posts/.test(lastDanceExport) || !/left join last_dance_engagements/.test(lastDanceExport) || !/\.cursor\(CURSOR_ROWS\)/.test(lastDanceExport)) failures.push("The Last Dance CSV is not complete, scalable, or protected against spreadsheet formula injection.");
if (!/DOWNLOAD GTD CARD/.test(lastDanceApp) || !/SHARE PASS ON X/.test(lastDanceApp) || !/LastDanceMintCountdown/.test(lastDanceApp) || !/GO TO OPENSEA MINT/.test(lastDanceApp) || !/lastDanceMintShareLine/.test(`${lastDanceApp}\n${lastDancePassPage}`) || !/bunnyhoodxyz\/overview/.test(lastDanceMint)) failures.push("The confirmed Last Dance entry is missing its download, dated X share, mint countdown, or exact OpenSea action.");
if (/wallet will be added soon/i.test(`${lastDanceApp}\n${lastDancePassPage}\n${lastDancePassCard}`)) failures.push("Removed Last Dance wallet-soon copy remains public or inside the downloadable card.");
